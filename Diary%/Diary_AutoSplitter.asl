...
init
{
    ...
    vars.hasSplitTriggered = false; // splitのフラグ
    vars.splitCounter = 0;         // splitをスキップする回数をカウント
}
...
split
{
    ...
    if (current.Loading > 4 && !vars.hasSplitTriggered)
    {
        ...
        {
            vars.splitCounter++;    // 最初の2回はカウントを増やすだけ
            return false;           // splitは発生させない
        }

        vars.hasSplitTriggered = true; // フラグを立てる
        return true;                   // splitを発生
    }

    // Loadingが4以下になった場合のリセット処理
    if (current.Loading <= 4)
    {
        vars.hasSplitTriggered = false; // フラグをリセット
    }
